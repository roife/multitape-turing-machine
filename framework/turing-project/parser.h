...
class TMParse {
    ...
    std::vector<TMachineDef::Transition::Direction> parse_tape_directions(
                                                                       size_t tape_num);
    TMachineDef::Transition
    parse_transition(TMachineDef::StateMap const &state_map, size_t tape_num,
                     std::unordered_set<TMachineDef::TapeSym> const &tape_alphabet,
                     TMachineDef::TapeSym blank_sym);
    std::unordered_multimap<TMachineDef::State, TMachineDef::Transition> parse_transitions(TMachineDef::StateMap const &state_map,
                                                                                     size_t tape_num,
                                                                                     std::unordered_set<TMachineDef::TapeSym> const &tape_alphabet,
                                                                                     TMachineDef::TapeSym blank_sym);
