...
#include <string>
#include <iostream>
#define parsing_assert(cond) if (!(cond)) { std::cerr << "syntax error" << std::endl; exit(ErrorCode::PARSING_ERROR); }
...
TMachineDef::State TMParse::parse_init_state(TMachineDef::StateMap const &state_map) {
    skip();
    expect_str("#q0 = ");
...
TMParse::parse_tape_directions(size_t tape_num)
{
    ...
    for (size_t i = 0; i < tape_num; ++i) {
        ...
        parsing_assert((tape_direction == 'l' || tape_direction == 'r' ||
                tape_direction == '*'));
        tape_directions.push_back(tape_direction == 'l' ? TMachineDef::Transition::Direction::LEFT
...
TMParse::parse_transitions(
    ...
    TMachineDef::TapeSym blank_sym)
{
    std::unordered_multimap<TMachineDef::State, TMachineDef::Transition> transitions;
    while (*cur_ptr) {
        auto transition =
