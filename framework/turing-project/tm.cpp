...
bool TMachineDef::Transition::match(State from, std::vector<TapeSym> read) const {
    if (this->from != from) {
        return false;
    }
...
void TMachine::run(bool verbose) {
    if (verbose) {
        ...
        std::string input(tapes[0].begin() + beg_end.first, tapes[0].begin() + beg_end.second + 1);
    ...
    auto beg_end = get_tape_range(tapes[0]);
    std::string result(tapes[0].begin() + beg_end.first, tapes[0].begin() + beg_end.second + 1);
    if (verbose) {
